...
const Dropdown = (props) => {
    ...
    const ulStyle = {
        backgroundColor:props.ulBgColor,
        color:props.ulColor
    }
    const liStyle = {
        "&:hover":{
            backgroundColor:props.ulColor
        }
    }
    return (
        <div className={`${styles["drop-down"]}`} onClick={handler} style={dropdownStyle}>
            <span style={{color:props.spanColor}}>{value}</span>
            {!props.data && <ul style={ulStyle}>
                <li value="Item 1" style={liStyle} onClick={selectHandler}>
                    Item 1
                </li>
                <li value="Item 2" style={liStyle} onClick={selectHandler}>
                    Item 2
                </li>
                <li value="Item 3" style={liStyle} onClick={selectHandler}>
                    Item 3
                </li>
            </ul>
            }
            {props.data && 
                <ul>
                    {props.data.map((item,index) => { 
                        return (
                            <li id={item.id} value={item.id} onClick={selectHandler} key={index}>
                                {item.name}
                            </li>
                        )
                    })}
